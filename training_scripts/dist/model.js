...
function initializeBackend() {
    // The backend will be automatically chosen by tfjs-node-gpu.
...
function createResidualBlock(inputTensor) {
    const initialConv = tf.layers.conv2d({ filters: CONV_FILTERS, kernelSize: 3, padding: 'same' }).apply(inputTensor);
    const bn1 = tf.layers.batchNormalization().apply(initialConv);
    const relu1 = tf.layers.reLU().apply(bn1);
    const nextConv = tf.layers.conv2d({ filters: CONV_FILTERS, kernelSize: 3, padding: 'same' }).apply(relu1);
...
function createDualResNetModel() {
    ...
    const initialConv = tf.layers.conv2d({ filters: CONV_FILTERS, kernelSize: 3, padding: 'same' }).apply(input);
    ...
    const policyConv = tf.layers.conv2d({ filters: 2, kernelSize: 1 }).apply(body);
    ...
    const valueConv = tf.layers.conv2d({ filters: 1, kernelSize: 1 }).apply(body);
